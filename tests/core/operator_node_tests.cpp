...
TEST_F(OperatorNodeTest, SetOperatorType_Changes) {
    ...
    node->set_operator_type(core::OperatorNode::OperatorType::kLogicalAnd);
...
TEST_F(OperatorNodeTest, BinaryOperator_HasTwoInputs) {
    ...
    node->set_operator_type(core::OperatorNode::OperatorType::kAddition);
    EXPECT_EQ(node->GetInputPinCount(), 2);

    node->set_operator_type(core::OperatorNode::OperatorType::kMultiplication);
    EXPECT_EQ(node->GetInputPinCount(), 2);

    node->set_operator_type(core::OperatorNode::OperatorType::kLogicalOr);
    EXPECT_EQ(node->GetInputPinCount(), 2);
}
...
TEST_F(OperatorNodeTest, UnaryOperator_HasOneInput) {
    ...
    node->set_operator_type(core::OperatorNode::OperatorType::kBitwiseNot);
    EXPECT_EQ(node->GetInputPinCount(), 1);

    node->set_operator_type(core::OperatorNode::OperatorType::kLogicalNot);
    EXPECT_EQ(node->GetInputPinCount(), 1);
}
...
TEST_F(OperatorNodeTest, AllOperators_HaveOneOutput) {
    ...
    node->set_operator_type(core::OperatorNode::OperatorType::kEqual);
    EXPECT_EQ(node->GetOutputPinCount(), 1);
}
...
TEST_F(OperatorNodeTest, ArithmeticOperators_ArithmeticCategory) {
    ...
    node->set_operator_type(core::OperatorNode::OperatorType::kDivision);
    EXPECT_EQ(node->GetCategory(), "Arithmetic");
}
...
TEST_F(OperatorNodeTest, BitwiseOperators_BitwiseCategory) {
    ...
    node->set_operator_type(core::OperatorNode::OperatorType::kBitwiseAnd);
...
TEST_F(OperatorNodeTest, ComparisonOperators_ComparisonCategory) {
    ...
    node->set_operator_type(core::OperatorNode::OperatorType::kLessThan);
    EXPECT_EQ(node->GetCategory(), "Comparison");
}
...
TEST_F(OperatorNodeTest, ConnectLiteralsToOperator_ValidTypes_Success) {
    auto* literal_a = graph_.AddNode<core::LiteralNode>(core::NodeBase::NodeKind::kLiteral);
...
TEST_F(OperatorNodeTest, ChainOperators_Success) {
    auto* literal_a = graph_.AddNode<core::LiteralNode>(core::NodeBase::NodeKind::kLiteral);
    auto* literal_b = graph_.AddNode<core::LiteralNode>(core::NodeBase::NodeKind::kLiteral);
    auto* literal_c = graph_.AddNode<core::LiteralNode>(core::NodeBase::NodeKind::kLiteral);
...
TEST_F(OperatorNodeTest, Deserialize_MissingFields_Fails) {
    ...
    core::Graph new_graph;
    auto* node = new_graph.AddNode<core::OperatorNode>(core::NodeBase::NodeKind::kOperator);
...
TEST_F(OperatorNodeTest, CanConnectTo_InvalidOutputPin_Fails) {
    auto* op_node = graph_.AddNode<core::OperatorNode>(core::NodeBase::NodeKind::kOperator);
    auto* target = graph_.AddNode<core::OperatorNode>(core::NodeBase::NodeKind::kOperator);
...
TEST_F(OperatorNodeTest, AllOperatorTypes_CanBeCreatedAndUsed) {
    ...
    for (auto op_type : all_ops) {
        auto* node = graph_.AddNode<core::OperatorNode>(core::NodeBase::NodeKind::kOperator);
        node->set_operator_type(op_type);
