...
class PrintNode : public NodeBase {
  public:
    ...
    std::expected<void, std::string> CanConnectTo(uint8_t out_pin, const NodeBase *target,
  ...
  private:
    ...
    PrintNode(uint32_t id, NodeKind kind, utils::WrappedVector2 position) noexcept;
