...
LoopNode::LoopNode(uint32_t id, NodeKind kind,
...
std::expected<void, std::string> LoopNode::CanConnectTo(
    ...
    if (target->GetInputPinType(in_pin) != GetOutputPinType(out_pin)) {
        return std::unexpected(
            std::string("Incompatible pin types for LoopNode output"));
    }
    return {};
}
...
std::expected<void, std::string> LoopNode::Deserialize(
