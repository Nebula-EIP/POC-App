...
bool GraphValidator::ValidationResult::HasPinConnectivityError() const {
    return std::any_of(errors.begin(), errors.end(),
                       [](const ValidationError &e) {
                           return e.type == ValidationError::ErrorType::kMissingRequiredInputPin ||
                                  e.type == ValidationError::ErrorType::kMissingRequiredOutputPin;
                       });
}
...
GraphValidator::ValidationResult GraphValidator::Validate(const Graph &graph) {
    ...
    result.errors.insert(result.errors.end(), cycle_errors.begin(), cycle_errors.end());
...
bool GraphValidator::DFSVisit(uint32_t node_id,
    ...
    for (const auto &child_conn : all_children) {
        ...
        if (colors[child_id] == DFSColor::kGray) {
            ...
            oss << "Cycle detected: node " << node_id << " (pin " << child_conn.out_pin
                << ") → node " << child_id << " (pin " << child_conn.in_pin << ")";
...
std::vector<GraphValidator::ValidationError> GraphValidator::CheckPinCompatibility(
    const Graph &graph) {
    std::vector<ValidationError> errors;
    ...
    for (const auto &node : all_nodes) {
        ...
        for (const auto &child_conn : all_children) {
            ...
            auto target_type = child_conn.node->GetInputPinType(child_conn.in_pin);

            // Check compatibility
            if (!AreTypesCompatible(static_cast<int>(source_type), static_cast<int>(target_type))) {
                ValidationError error;
                error.type = ValidationError::ErrorType::kIncompatiblePinTypes;

                std::ostringstream oss;
                oss << "Pin type mismatch: " << PinTypeToString(static_cast<int>(source_type)) << " (node "
                    << node->id() << ", pin " << static_cast<int>(child_conn.out_pin)
                    << ") → " << PinTypeToString(static_cast<int>(target_type)) << " (node "
                    << child_conn.node->id() << ", pin "
                ...
                error.involved_pins.push_back({child_conn.node->id(), child_conn.in_pin});
...
std::vector<GraphValidator::ValidationError> GraphValidator::CheckPinConnectivity(
    const Graph &graph) {
    std::vector<ValidationError> errors;
    ...
    for (const auto &node : all_nodes) {
        ...
        for (uint8_t pin = 0; pin < input_count; ++pin) {
            // Note: For now, we check all input pins. Some nodes may have optional
            // inputs, but that's a more advanced feature.
            if (!node->IsInputPinConnected(pin)) {
                // Check if this node type requires this pin to be connected
                // For now, we only check for nodes that explicitly require connections
                // (e.g., OperatorNode, but not FunctionInputNode which can be unconstrained)

                auto kind = node->kind();
                bool requires_connection = false;

                // Define which node types require all input pins to be connected
                ...
                if (requires_connection) {
                    ValidationError error;
                    error.type = ValidationError::ErrorType::kMissingRequiredInputPin;

                    std::ostringstream oss;
                    oss << "Required input pin not connected: node " << node->id() << ", pin "
                        << static_cast<int>(pin) << " (" << node->GetInputPinName(pin) << ")";
                    error.message = oss.str();
        ...
        for (uint8_t pin = 0; pin < output_count; ++pin) {
            if (!node->IsOutputPinConnected(pin)) {
                // Note: In many cases, unused outputs are acceptable (dead code)
                // Only enforce this for nodes where outputs are required to be used
                auto kind = node->kind();
                ...
                if (requires_usage) {
                    ValidationError error;
                    error.type = ValidationError::ErrorType::kMissingRequiredOutputPin;

                    std::ostringstream oss;
                    oss << "Output pin should be used: node " << node->id() << ", pin "
                        << static_cast<int>(pin) << " (" << node->GetOutputPinName(pin) << ")";
                    error.message = oss.str();
