...
void CodegenOptimizer::MarkAsUsed(uint32_t node_id, const core::Graph &graph,
...
std::unordered_set<uint32_t> CodegenOptimizer::FindUsedNodes(
...
void CodegenOptimizer::InferTypes(
    const core::Graph &graph,
...
DeadCodeAnalysis CodegenOptimizer::AnalyzeGraph(const core::Graph &graph) {
    ...
    for (const auto &node_ptr : all_nodes) {
        ...
        analysis.type_info[node_id] = TypeInfo{
            type_map[node_id],
            dynamic_cast<const core::LiteralNode *>(node) != nullptr, is_used};
    }
