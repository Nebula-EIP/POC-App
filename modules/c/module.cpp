...
std::string_view CModule::Name() const noexcept { return kName; }

std::string_view CModule::Author() const noexcept { return kAuthor; }

std::string_view CModule::Description() const noexcept { return kDescription; }

core::IModule::Version CModule::GetVersion() const noexcept { return kVersion; }
...
core::ModuleId CModule::Id() const noexcept { return id_; }

core::capa::ITypeListCapability *CModule::Types() { return types_.get(); }

core::capa::INodeListCapability *CModule::Nodes() { return nodes_.get(); }
...
core::ICapability *CModule::Capability(std::type_index type) noexcept {
    if (type == std::type_index(typeid(core::capa::ITypeListCapability))) {
        return types_.get();
               ...
               std::type_index(typeid(core::capa::INodeListCapability))) {
        return nodes_.get();
    } else if (type ==
               std::type_index(typeid(core::capa::IRendererCapability))) {
        return renderer_.get();
    } else if (type ==
               std::type_index(typeid(core::capa::IImporterCapability))) {
        return importer_.get();
    } else if (type ==
               std::type_index(typeid(core::capa::IExporterCapability))) {
...
const core::ICapability *CModule::Capability(
